...
public class DemoSnakeColourHandler_Script : MonoBehaviour
{
    //this script handles colouring the demo snake 
    // includes the original snake colours + gold flashing + ghost opacity flashing
    private DemoSnake_Script snakeScript;
    private Color playerColour;
    private Color retroSnakeColour;
    ...
    public void Setup(DemoSnake_Script script, Color playerColour, Color _retroSnakeColour, SpriteRenderer headRenderer)
    {
        snakeScript = script;
        ...
        retroSnakeColour = _retroSnakeColour;

        renderers = new List<SpriteRenderer>(); 
        AddRenderer(headRenderer, true);
    }

    public void ShowEel(bool isEel){
        for (int i = 0; i < renderers.Count; i++)
        {
            // hide all the retro snake sprites
            renderers[i].transform.GetChild(0).gameObject.SetActive(!isEel);
            // enable all the eel sprites
            renderers[i].enabled = isEel;
        }
    }

    public void AddRenderer(SpriteRenderer newRenderer, bool isEel)
    {
        ...
        newRenderer.transform.GetChild(0).GetComponent<SpriteRenderer>().color = retroSnakeColour;

        newRenderer.transform.GetChild(0).gameObject.SetActive(!isEel);
        newRenderer.enabled = isEel;
    }
 
    public void ClearSegments(bool isEel)//resets renderers list except for snake's head
    {
        SpriteRenderer headRenderer = renderers[0];
        renderers.Clear();
        AddRenderer(headRenderer, isEel);
    }
